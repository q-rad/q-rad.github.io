...
In high-reliability sectors such as space, aerospace, and defense, the quality of each component directly affects overall system performance and mission success.
...
The table below is an excerpt from NASA’s EEE-INST-002 that outlines component-level test matrices.
...
Active components must undergo radiation-hardness evaluation and, if no applicable data exist, dedicated radiation testing is required.
...
Applies to missions with moderate reliability requirements that balance performance, cost, and risk.
...
Includes components with higher or unknown risk due to limited screening or qualification.
These parts often lack reliable data or flight heritage, and lot-to-lot process variation can invalidate prior results.
Commonly used in CubeSats or low-cost experimental missions.
Radiation review and testing are required if data are unavailable.
Typical mission duration: less than 2 years.
</details>

---

The European Space Agency (ESA) defines Class 1 through Class 3 categories and provides guidance on supplementing up-screening when using AEC-Q–qualified components.
