...
However, some flip-chip BGA packages allow laser access from the underside of the die, enabling evaluation.
Because the same silicon can be offered in multiple package options, migrating to a more laser-compatible package may make evaluation feasible.
