...
QRT가 보유한 레이저 장비는 650~1650nm의 파장을 지원하여 Si, GaN, SiC등 일반적인 모든 반도체 소재뿐만 아니라 메모리, 프로세서, 아날로그 소자 등도 평가 가능합니다. 하지만 HBM의 3D 스택 같은 특수한 케이스의 경우 완전한 평가가 불가능합니다. (레이저가 스택을 투과할 수 없기 때문에)<br>
이런 특수한 케이스를 제외하더라도 일부 패키지 타입에 따라 평가가 제한될 수 있습니다.
레이저 평가를 위해서는 소자의 Epoxy Mold Compound를 제거하는 De-capsulation이 필수적입니다. 
Amplifier, ADC, isolator 등 단순 소자의 경우 Back-Side 뿐 아니라 Front-Side 를 통한 레이저 평가가 가능합니다.
하지만 FPGA, MCU, Memory(SRAM,DRAM)등의 복잡한 소자의 경우 Back-Side로 만 평가가 가능합니다.
<br>
...
<p align= "center">
<img src="/assets/Articles/메모리1.png" style="width: 100%; max-width: 300px;" alt= "De-capsulation 된 TSSOP 소자">
</p>
...
<p align= "center">
<img src="/assets/Articles/평가불가능소자.png" style="width: 100%; max-width: 400px;" alt= "Sub-Strate type package">
</p>
...
위의 설명 만으로는 정확히 평가하려는 소자가 레이저로 평가 가능할지 알기 어려우실 겁니다.
대부분의 경우 X-ray (엑스레이)를 찍어보면 평가가 가능 할지 알아볼 수 있습니다.
...
완벽한 평가를 위해서는 Lead Frame을 제거해야 했지만, 엑스레이로 확인시 Lead Frame이 여러 핀들과 연결되어 있어 제거가 불가능 했습니다.
다만 Lead Frame 사이의 접착제가 제거된 부분으로 일부분 평가가 가능하였고 방사선으로 인한 에러가 발생할 가능성을 예측하였습니다.
...
BGA(Ball Grid Array) 패키지는 일반적인 평가 접근이 어려운 구조이나, 해당 소자는 Flip-Chip 공정이 적용되어 있어 후면에 대해 손쉽게 평가 수행이 가능하였습니다
...
Lead Frame이 칩 후면부 전체를 덮고 있는 형태라서 제거가 필요하였습니다. 칩 제작 업체와 정밀하게 촬영한 엑스레이를 통해서 제거 가능한 Lead Frame 영역을 확인하였고, Lead Frame과의 연결에 손상이 적게 정밀하게 제거함으로써 전체 칩 면적의 90% 이상에 대해 평가가 가능하였습니다.
...
<p align= "center">Op-Amp 전면(좌),후면(우) (SOIC)</p>
...
<p align= "center">ADC 전면(좌),후면(우) (PDIP)</p>
...
<p align= "center">Digital Isolator 전면(좌),후면(우) (SOIC)</p>
